...
def gillespie(kc, kd, ktra, kd4, c, rib, crib, aa, p, truncp, t):
    ...
    while t < timespan:  #                
        ...
        reactiona = kc*c*rib # crib+1, c-1, rib-1
        reactionb = kd*crib # crib-1, c+1, rib+1
        reactionc = ktra*crib #  aa+1
        reactiond = kd4*p # p-1
        reactione = kd4*truncp # p-1
...
print("steady state of protein is: ", p_average_list[-1])
print("steady state of truncated protein is: ", tp_average_list[-1])
...
#get hisotrgam of size of trucnated proteins
plt.hist(simulations[+6], density=False, bins=10, color="dodgerblue")  # `density=False` would make counts
plt.hist(simulations[+13], density=False, bins=10, color="dodgerblue")  # `density=False` would make counts
plt.hist(simulations[+20], density=False, bins=10, color="dodgerblue")  # `density=False` would make counts
plt.hist(simulations[+27], density=False, bins=10, color="dodgerblue")  # `density=False` would make counts
plt.hist(simulations[+34], density=False, bins=10, color="dodgerblue")  # `density=False` would make counts
plt.ylabel('Frequency')
plt.xlabel('Truncated protein length (aa)');
plt.show()


###

# #TO GET RIBOSOME:MRNA ASSOCIATION GRAPHS, SET timespan TO 100 and uncomment code beow and comment out graph code above
